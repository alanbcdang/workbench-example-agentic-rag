...
"""
GB10 Modified Graph - Uses Open Source Search Instead of Tavily
This version replaces Tavily with DuckDuckGo or SearXNG for GB10 compatibility
"""
...
# MODIFIED: Use open source search instead of Tavily
from chatui.utils.opensource_search import get_search_tool

from chatui.utils import database, nim


# MODIFIED: Open source search parameters (replaces Tavily)
DEFAULT_SEARCH_K = 3
SEARCH_K = int(os.getenv("SEARCH_K", DEFAULT_SEARCH_K))
SEARCH_ENGINE = os.getenv("SEARCH_ENGINE", "duckduckgo")

class SearchAPIError(Exception):
    """Raised when search returns invalid results."""
    pass
...
def web_search(state):
    ...
    MODIFIED FOR GB10: Uses open source search (DuckDuckGo or SearXNG) instead of Tavily
    ...
    print(f"---WEB SEARCH ({SEARCH_ENGINE.upper()})---")
    question = state["question"]
    documents = state.get("documents", [])

    # MODIFIED: Use open source search instead of Tavily
    try:
        web_search_tool = get_search_tool(engine=SEARCH_ENGINE)

        # Perform search
        search_results = web_search_tool.search.search(
            query=question,
        ...
        if not isinstance(search_results, list):
            raise SearchAPIError(f"Invalid response from {SEARCH_ENGINE}: {search_results}")

        if not search_results:
            print(f"---WARNING: {SEARCH_ENGINE.upper()} returned no results---")
            return {"documents": documents, "question": question}
        ...
        for i, result in enumerate(search_results, 1):
            ...
            web_content.append(f"[Result {i}]")
            if title:
                web_content.append(f"Title: {title}")
            if url:
                web_content.append(f"URL: {url}")
            if content:
                web_content.append(f"Content: {content}")
            web_content.append("")  # Empty line between results
        ...
        raise SearchAPIError(f"{SEARCH_ENGINE} web search failed: {e}")
